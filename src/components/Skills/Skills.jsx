          ...
          >
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 w-full justify-items-center">
              {category.skills.map((skill) => (
                <div
                  key={skill.name}
                  className="flex items-center justify-center flex-wrap space-x-2 bg-transparent border-2 border-gray-700 rounded-3xl py-2 px-4 sm:px-5 text-center text-sm sm:text-base hover:border-[#8245ec] transition-all duration-300"
                >
                  <img
                    src={skill.logo}
                    alt={`${skill.name} logo`}
